 ...
 */
private unsigned long long l1_clock(struct l1_global *l1){
	struct timeval now;
 ...
 */
private struct l1_socket *l1_socket_add(struct l1_global *l1, int skt,
				...
				char *descr){
	struct l1_socket *ls = (struct l1_socket *) dcalloc(1, sizeof(*ls));
	...
	if (getsockopt(skt, SOL_SOCKET, SO_SNDBUF, &ls->sndbuf_size, &optlen) < 0) {
		perror("l1_socket_add: getsockopt SO_SNDBUF");
	}
	else {
		printf("l1_socket_add %s: sndbuf %d\n", descr, ls->sndbuf_size);
	}
	optlen = sizeof(ls->rcvbuf_size);
	if (getsockopt(skt, SOL_SOCKET, SO_RCVBUF, &ls->rcvbuf_size, &optlen) < 0) {
		perror("l1_socket_add: getsockopt SO_RCVBUF");
	}
	else {
		printf("l1_socket_add %s: rcvbuf %d\n", descr, ls->rcvbuf_size);
	}
	if (ls->sndbuf_size == 0) {
	...
	if (epoll_ctl(l1->epoll, EPOLL_CTL_ADD, skt, &ee) > 0) {
		perror("l1_socket_add: epoll_ctl");
	}
#endif /* USE_EPOLL */
...
struct l1_global *l1_init(int id,
	...
	struct l1_global *l1 = (struct l1_global *) dcalloc(1, sizeof(*l1));
	
	l1->id = id;
	l1->outgoing = outgoing;
	l1->deliver = deliver;
#ifdef USE_EPOLL
	if ((l1->epoll = epoll_create(100)) < 0) {
		perror("l1_init: epoll_create");
	}
#else
	l1->epoll = -1;
#endif
 ...
 */
void l1_socket_send(struct l1_global *l1, struct l1_socket *ls,
				const char *data, unsigned int size, void (*release)(void *), void *arg){
#ifdef USE_DEBUG
	printf("l1_socket_send: %d\n", size);
#endif

	struct l1_chunk_queue *cq = (struct l1_chunk_queue *) dcalloc(1, sizeof(*cq));
	cq->data = data;
	cq->size = size;
	cq->release = release;
	cq->arg = arg;
	...
	if (ls->remainder == 0) {
		...
		memset(&ee, 0, sizeof(ee));
		ee.data.ptr = ls;
		ee.events = EPOLLIN | EPOLLHUP | EPOLLOUT;
		if (epoll_ctl(l1->epoll, EPOLL_CTL_MOD, ls->skt, &ee) > 0) {
			perror("l1_socket_send: epoll_ctl");
		}
	}
#endif /* USE_EPOLL */
	*ls->cqlast = cq;
 ...
 */
private int l1_send_ready(struct l1_global *l1, struct l1_socket *ls){
#ifdef USE_DEBUG
	...
	/* If there's room in the send buffer, poll the layer above if it is so capable.
	 */
	...
	if (iovlen > 0) {
...
#ifdef USE_DEBUG
		fprintf(stderr, "l1_send_ready: sent %d bytes out of %d to socket %d (%s) %d\n",
		...
		if (n <= 0) {
			...
			if (ls->remainder == 0) {
				...
				ee.events = EPOLLIN | EPOLLHUP;
				if (epoll_ctl(l1->epoll, EPOLL_CTL_MOD, ls->skt, &ee) > 0) {
					perror("l1_send_ready: epoll_ctl");
				}
			}
#endif /* USE_EPOLL */
		}
 ...
 * itself).
 */
private int l1_recv_ready(struct l1_global *l1, struct l1_socket *ls){
	if (ls->chunk == 0) {
	...
	if (size == ls->received) {
		...
		(*ls->deliver)(l1, ls, ls->chunk + 4, size - 4, l1_release_chunk, ls->chunk);
		ls->chunk = 0;
		return 1;
	}
 ...
 */
private int l1_gotclient(struct l1_global *l1, struct l1_socket *ls){
	int clt;
	...
	l1_socket_add(l1, clt, l1_recv_ready,
 ...
 */
void l1_listen(struct l1_global *l1, unsigned int port){
	...
	l1_socket_add(l1, skt, l1_gotclient, 0, 0, "l1_listen TCP");
}
...
void l1_connect(struct l1_global *l1, const char *host, unsigned int port){
	struct sockaddr_in sin;
	struct l1_addrlist *al;
	 ...
	 */
	al = (struct l1_addrlist *) dcalloc(1, sizeof(*al));
	al->sin = sin;
	al->next = l1->addrs;
	l1->addrs = al;
}
 ...
 */
void l1_try_connect(struct l1_global *l1){
	struct l1_addrlist *al;
	...
	for (al = l1->addrs; al != 0; al = al->next) {
		if (al->out == 0) {
			...
			/* Start connect.
			 */
			if (connect(skt, (struct sockaddr *) &al->sin,
											sizeof(al->sin)) < 0) {
				extern int errno;
			...
			/* Register the socket.
			 */
			al->out = l1_socket_add(l1, skt, l1_recv_ready,
...
void l1_send(struct l1_global *l1, struct l1_socket *ls,
	...
	hdr[1] = (len >> 8) & 0xFF;
	hdr[2] = (len >> 16) & 0xFF;
	l1_socket_send(l1, ls, hdr, 4, l1_release_chunk, hdr);
	l1_socket_send(l1, ls, bytes, len - 4, upcall, env);
}

/* Close the given connection.
 */
void l1_close(struct l1_conn *conn){
}
 ...
 */
void l1_prepare_poll(struct l1_global *l1, struct pollfd *fds){
	unsigned int i;
	short int events;
	struct l1_socket *ls;
 ...
 */
private int l1_poll(struct l1_global *l1, unsigned long long start,
				unsigned long long now, int timeout, struct pollfd *fds){
...
#ifdef USE_EPOLL
	if (l1->epoll >= 0) {
		int n = epoll_wait(l1->epoll, l1->events, MAX_EPOLL_EVENTS, timeout);
		if (n < 0) {
			if (errno != EINTR) {
				perror("l1_poll: epoll");
			}
			else {
				n = 0;
			}
 ...
 */
private int l1_handle_events(struct l1_global *l1, struct pollfd *fds, int n){
	...
	if (l1->epoll >= 0) {
		for (i = 0; i < n; i++) {
			ls = l1->events[i].data.ptr;
			unsigned int events = l1->events[i].events;
			if (events & (EPOLLIN | EPOLLHUP)) {
				if (!(*ls->input)(l1, ls)) {
					printf("l1_handle_events: closing socket %d\n", ls->skt);
					close(ls->skt);
					ls->skt = -1;
					closed_sockets = 1;
				}
 ...
 */
private void l1_socket_release(struct l1_global *l1, struct l1_socket *ls){
 ...
 */
private void remove_closed_sockets(struct l1_global *l1){
	struct l1_socket **pls, *ls;
	for (pls = &l1->sockets; (ls = *pls) != 0;) {
		if (ls->skt == -1) {
			*pls = ls->next;
			l1_socket_release(l1, ls);
		}
		else {
			pls = &ls->next;
		}
 ...
 */
int l1_wait(struct l1_global *l1, int timeout){
	unsigned long long start = l1_clock(l1);
