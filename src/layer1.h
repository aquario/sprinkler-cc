...
struct l1_global {
	int id;										// id of proxy
	struct l1_socket *sockets;					// linked list of sockets
	unsigned int nsockets;						// #sockets in the list
	struct l1_addrlist *addrs;					// addresses of peers
	int epoll;									// epoll fd (-1 means no epoll)
	struct timeval starttime;					// time at which l1_init was invoked
	unsigned long long time_to_attempt_connect;	// periodically attempting connects
...
struct l1_socket {
	...
	void (*deliver)(struct l1_global *, struct l1_socket *, const char *, unsigned int,
													void (*)(void *), void *);
